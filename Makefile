...
#======= Image builds =======
$(PGOROOT)/build/%/Dockerfile_:
	$(error No Dockerfile_ found for $* naming pattern: [$@])

%-img-build: pgo-base-$(IMGBUILDER) build-% $(PGOROOT)/build/%/Dockerfile_
	$(IMGCMDSTEM) \
		-f $(PGOROOT)/build/$*/Dockerfile_ \
...
pgo-base-build: $(PGOROOT)/build/pgo-base/Dockerfile_ licenses
	$(IMGCMDSTEM) \
		-f $(PGOROOT)/build/pgo-base/Dockerfile_ \
